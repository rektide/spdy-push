...
Push.prototype._acknowledge = function () {
  ...
  this.stream = new Promise(function(resolve, reject){
    self.res.push(self.path, {request:{}, response: self.headers}, function(err, stream) {
      if (err) reject(err)
      resolve(stream)
    })
  })
  return stream.then(function(stream){
    return new Promise(function (resolve, reject) {
      stream.on('acknowledge', acknowledge) // no longer in node-spdy
...
Push.prototype._send = function () {
  var self = this
  return self.stream.then(function (stream) {
